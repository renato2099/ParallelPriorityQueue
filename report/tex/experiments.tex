...
\end{figure}
The previous experiments show that the same workload performs on a Xeon CPU roughly a magnitude better than on the Xeon Phi architecture. There are multiple reasons for these, ...\\ %cache coherence, memory allocation is more expensive, ....
We assume the main drawback on the Xeon Phi architecture is that any cache-line invalidation might lead to many cores ending up with dirty cache lines. The more hardware core are used on a concurrent application the higher is the probability that at least two threads share a cache line. All this dirty cache lines lead to overhead due to that cache-coherence protocol or refetching of cache-lines.\\
This experiment should check how the Xeon Phi would perform if not a single cache-line gets invalidated when the lock-free implementation is accessed. Therefore we implemented an additional \textit{contains} function which only looks up a specific element but makes no changes to the structure of the priority queue. \figurename~\ref{fig:comp_contains}  shows the lock-free priority queue running on an Intel Core i7-3820, Xeon CPU and the Xeon Phi. For all runs the queue was prepopulated with 10\,Mio. elements, each threads executed 100'000 \textit{contains} operations. As can be seen in the graph the runtime on the Xeon Phi only increases slightly with the number of threads, in contrast on the Intel Core i7-3820 it increases roughly linearly. This means that on the x86 architecture the \textit{contains} operation is memory bound already with a single thread. The Xeon Phi seems to guarantee a fixed bandwidth for each of its hardware cores, such that we can scale the number of threads up without being limited by the memory bandwidth.\\
